...
/*
	...
	$RCSfile: threadexpiries_daily.inc.php,v $
	$Revision: 1.5 $
	$Date: 2006/02/28 03:00:19 $
*/
...
if($actionarray) {
	...
	foreach($actionarray as $action => $tids) {
		...
		switch($action) {
			...
			case 'UES':
				...
				$db->query("UPDATE {$tablepre}threadsmod SET status='0' WHERE tid IN ($tids) AND action='EST'", 'UNBUFFERED');
			...
			case 'UEH':
				...
				$db->query("UPDATE {$tablepre}threadsmod SET status='0' WHERE tid IN ($tids) AND action='EHL'", 'UNBUFFERED');
				break;
			...
			case 'UEO':
				...
				$db->query("UPDATE {$tablepre}threadsmod SET status='0' WHERE tid IN ($tids) AND action IN ('EOP', 'ECL')", 'UNBUFFERED');
				break;
	...
	//$db->query("DELETE FROM {$tablepre}threadexpiries WHERE expiration<$timestamp");
}
