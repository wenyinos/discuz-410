...
<?php if(is_array($extcredits)) { foreach($extcredits as $id => $credit) { ?>
...
<?php if($qihoo_status && $searchboxstatus) { ?>
<tr class="altbg2"><td>
<table width="100%" border="0" cellpadding="1" cellspacing="0">
<tr><td width="50%">
<?php if($qihoo_links['keywords']) { ?>
<span class="bold">热门搜索</span>&nbsp;
<?php if(is_array($qihoo_links['keywords'])) { foreach($qihoo_links['keywords'] as $link) { ?>
<?=$link?>&nbsp;
<?php } } } ?>
</td><td align="right">
...
<tr><td>
<?php if($qihoo_links['topics']) { ?>
<span class="bold">论坛专题</span>&nbsp;
<?php if(is_array($qihoo_links['topics'])) { foreach($qihoo_links['topics'] as $url) { ?>
<?=$url?> &nbsp;
<?php } } } ?>
</td><td align="right">
<?php if($customtopics) { ?>
...
<?php } if(is_array($forumlist)) { foreach($forumlist as $key => $forum) { if($forum['type'] == 'group' && $forumlist[($key + 1)]['type'] == 'forum') { if($key) { ?>
</tbody></table><br></div>
<?php } ?>
<div class="maintable">
<table cellspacing="<?=INNERBORDERWIDTH?>" cellpadding="<?=TABLESPACE?>" width="<?=TABLEWIDTH?>" align="center" class="tableborder">
...
<?php } else { if(is_array($forum['lastpost'])) { ?>
...
<?php } } } if(empty($gid) && ($_DCACHE['forumlinks'] || $whosonlinestatus || $bdaystatus == 1 || $bdaystatus == 3)) { if(empty($forumlist)) { ?>
<br><br>
<?php } else { ?>
</table><br></div><div class="maintable">
<?php } ?>
<table cellspacing="<?=INNERBORDERWIDTH?>" cellpadding="<?=TABLESPACE?>" width="<?=TABLEWIDTH?>" align="center" class="tableborder">
<?php } if(empty($gid)) { if($_DCACHE['forumlinks']) { ?>
...
<?php if(is_array($_DCACHE['forumlinks'])) { foreach($_DCACHE['forumlinks'] as $flink) { if($flink['type'] == 1) { ?>
<tr>
...
<?php } if($bdaystatus == 1 || $bdaystatus == 3) { ?>
...
<?php if($_DCACHE['settings']['todaysbdays']) { ?>
...
<?php } if($whosonlinestatus) { if($detailstatus) { ?>
...
<?php if($whosonline) { if(is_array($whosonline)) { foreach($whosonline as $key => $online) { if($key % 7 == 0) { ?>
