...
<?php if($allowmodpost && $forum['modnewposts']) { ?>
...
<?php } if(!empty($newpmexists)) { ?>
<div class="maintable">
<?php include template('pmprompt'); ?>
</div>
<?php } if($subexists) { ?>
<div class="maintable">
<?php include template('forumdisplay_subforum'); ?>
</div>
...
<?php } if($forum['threadtypes'] && $forum['threadtypes']['listable']) { ?>
...
<?php if(is_array($forum['threadtypes']['types'])) { foreach($forum['threadtypes']['types'] as $id => $name) { if($typeid != $id) { ?>
...
<?php if($page == 1 && !empty($announcement)) { ?>
<tr>
...
<?php if($threadcount) { if(is_array($threadlist)) { foreach($threadlist as $key => $thread) { ?>
<div class="maintable">
...
<?php } if($forum['ismoderator']) { if($thread['fid'] == $fid) { ?>
...
<?php } if($thread['moved']) { if($forum['ismoderator']) { ?>
<a href="topicadmin.php?action=delete&tid=<?=$thread['moved']?>">移动:</a>
<?php } else { ?>
移动:
<?php } } elseif($thread['digest']) { ?>
<img src="<?=IMGDIR?>/digest.gif" align="absmiddle"> 精华<b>
<?php echo substr('III', - $thread['digest']); ?>
</b>:&nbsp;
<?php } elseif($thread['displayorder']) { ?>
<img src="<?=IMGDIR?>/pin.gif" align="absmiddle"> 置顶<b>
<?php echo substr('III', - $thread['displayorder']); ?>
</b>:&nbsp;
<?php } elseif($thread['poll']) { ?>
...
<?php } if($forum['ismoderator'] && $threadcount) { ?>
<div class="maintable">
...
<?php if($allowuseblog && $forum['allowblog']) { ?>
...
<tr><td nowrap>
<?php if(is_array($whosonline)) { foreach($whosonline as $key => $online) { if($key % 7 == 0) { ?>
...
<?php if($filter == 'digest' || $filter == 'type') { ?>
