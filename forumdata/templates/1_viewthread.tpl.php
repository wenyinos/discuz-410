<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); include template('header'); ?>
...
<?php } if($polloptions) { ?>
<div class="maintable">
<?php include template('viewthread_poll'); ?>
</div>
<?php } if($newpmexists) { ?>
<div class="maintable">
<?php include template('pmprompt'); ?>
</div>
<?php } ?>
<div class="maintable">
...
<?php if($lastmod['modaction'] || $thread['blog'] || $thread['readperm'] || $thread['price'] > 0) { ?>
<tr class="category"><td colspan="2" align="center" class="bold">
<?php if($lastmod['modaction']) { ?>
...
<?php if($forum['ismoderator']) { if($allowviewip) { ?>
...
<?=$post['authortitle']?>
<br>
<?php showstars($post['stars']); ?>
<br>
<?php if($post['customstatus']) { ?>
<?=$post['customstatus']?><br>
<?php } ?>
<br>
<?php if($avatarshowpos == 3 && $post['avatarshow']) { ?>
...
<?php } else { ?>
<br><br>
<?php } if($post['medals']) { if(is_array($post['medals'])) { foreach($post['medals'] as $medal) { ?>
...
积分 <?=$post['credits']?><br>
帖子 <?=$post['posts']?><br>
<?php if(is_array($extcredits_thread)) { foreach($extcredits_thread as $key => $credit) { ?>
<?=$credit['title']?> <?=$post[$key]?> <?=$credit['unit']?><br>
<?php } } ?>
阅读权限 <?=$post['readaccess']?><br>
注册 <?=$post['regdate']?>
<?php if(is_array($_DCACHE['fields_thread'])) { foreach($_DCACHE['fields_thread'] as $field) { ?>
<br><?=$field['title']?>
<?php if($field['selective']) { ?>
...
<?php } ?>
<br>
<?php if($vtonlinestatus && $post['authorid']) { if($timestamp - $post['lastactivity'] <= 10800 && !$post['invisible']) { ?>
状态 <b>在线</b>
<?php } else { ?>
状态 离线
<?php } } if($avatarshowpos == 2) { ?>
...
<?php if(!empty($advlist['thread'][$post['count']])) { ?>
...
<?php } if($bannedmessages && (($post['authorid'] && !$post['username']) || ($post['groupid'] == 4 || $post['groupid'] == 5))) { ?>
*** 作者被禁止或删除 内容自动屏蔽 ***
<?php } else { ?>
...
<?php } else { if(is_array($post['attachments'])) { foreach($post['attachments'] as $attach) { ?>
<br><br><?=$attach['attachicon']?>
<?php if($attach['attachimg']) { ?>
<a href="member.php?action=credits&view=getattach" title="查看积分策略说明" target="_blank">图片附件</a>:
<?php if($attach['readperm']) { ?>
...
<?php } } } } if($post['number'] == 1 && $relatedkeywords) { ?>
<br><br><span class="bold">相关关键字:</span> <?=$relatedkeywords?><br><br><br>
<?php } if($post['signature'] && !$post['anonymous']) { ?>
...
<tr><td align="left">
<?php if($forum['ismoderator'] && $allowdelpost) { if($post['number'] == 1) { ?>
...
<?php if($post['username'] && !$post['anonymous']) { ?>
...
<?php if($forum['ismoderator'] || $post['authorid'] == $discuz_uid) { ?>
...
<?php if(is_array($relatedthreadlist)) { foreach($relatedthreadlist as $key => $threads) { if($threads['title']) { ?>
<tr>
...
<?php echo rawurlencode($threads['author']); ?>
...
<?php if($allowanonymous || $forum['allowanonymous']) { ?>
...
<?php if($thread['authorid'] && ($thread['authorid'] == $discuz_uid || $forum['ismoderator'])) { if($thread['blog']) { ?>
 | <a href="misc.php?action=blog&tid=<?=$tid?>">移除 Blog</a>
<?php } elseif($allowuseblog && $forum['allowblog'] && $thread['authorid'] == $discuz_uid) { ?>
...
<?php if($allowstickthread) { ?>
<option value="stick">主题置顶</option>
<?php } if($thread['price'] > 0 && $allowrefund) { ?>
...
<?php include template('footer'); if($relatedthreadupdate) { ?>
